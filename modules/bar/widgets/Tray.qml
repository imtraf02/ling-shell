  ...
  function _performFilteredItemsUpdate() {
    ...
    if (SystemTray.items && SystemTray.items.values) {
      ...
      for (let i = 0; i < trayItems.length; i++) {
        ...
        for (let j = 0; j < Settings.bar.tray.blacklist.length; j++) {
          if (wildCardMatch(title, Settings.bar.tray.blacklist[j])) {
            isBlacklisted = true;
            break;
          }
        }
    ...
    if (Settings.bar.tray.favorites.length > 0) {
      let fav = [];
      for (let item of newItems) {
        const title = item.tooltipTitle || item.name || item.id || "";
        for (let rule of Settings.bar.tray.favorites) {
          ...
          MouseArea {
            ...
            onClicked: mouse => {
              if (!trayItem.modelData)
                return;
              const menuPanel = PanelService.getPanel("panel:tray-menu", root.screen);
