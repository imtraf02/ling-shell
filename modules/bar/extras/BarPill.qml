...
Item {
  ...
  readonly property int textSize: Math.max(1, Math.round(pillHeight * 0.33))
  readonly property int iconSize: Math.max(1, Math.round(pillHeight * 0.48))
  ...
  Rectangle {
    ...
    IText {
      ...
      x: {
        ...
        var offset = root.oppositeDirection ? Style.padding.small : -Style.padding.small;
        if (root.forceOpen) {
          // If its force open, the icon disc background is the same color as the bg pill move text slightly
          offset += root.oppositeDirection ? -Style.padding.small : Style.padding.small;
        }
