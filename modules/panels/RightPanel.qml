...
Item {
  ...
  Item {
    ...
    Item {
      ...
      readonly property real currentWidth: root.isClosing ? 0 : (root.isPanelVisible ? targetWidth : 0)

      readonly property real currentHeight: targetHeight

      property int topLeftCornerState: 0
      property int topRightCornerState: (x + width >= root.width - 1) ? 2 : 0
      property int bottomLeftCornerState: 0
      property int bottomRightCornerState: (x + width >= root.width - 1) ? 2 : 0
      ...
      Behavior on width {
        ...
        NumberAnimation {
          ...
          onRunningChanged: {
            if (!running) {
              if (root.isClosing && panelBackground.width === 0) {
                Qt.callLater(root._finalizeClose);
              }
            }
          }
