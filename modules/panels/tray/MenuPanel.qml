...
SmartPanel {
  ...
  function addToFavorites() {
    ...
    if (!itemName)
      return;
    const favorites = Settings.bar.tray.favorites || [];
    Settings.bar.tray.favorites = favorites.concat(itemName);
  ...
  function removeFromFavorites() {
    ...
    if (!itemName)
      return;
    const favorites = Settings.bar.tray.favorites || [];
    Settings.bar.tray.favorites = favorites.filter(f => f !== itemName);
    Settings.saveImmediate();
  ...
  panelContent: Item {
    ...
    property real contentPreferredWidth: root.trayWidth
    property real contentPreferredHeight: {
      if (activeSubMenuItems.length > 0) {
        ...
        return Math.min(maxH, subHeight + (Style.padding.small * 2));
      }

      const mainHeight = mainFlickable.contentHeight;
      const maxH = (root.screen ? root.screen.height : Screen.height) * 0.9;
      return Math.min(maxH, mainHeight + (Style.padding.small * 2));
    }
            ...
            delegate: Rectangle {
              ...
              Rectangle {
                ...
                RowLayout {
                  ...
                  IconImage {
                    implicitSize: Style.padding.small
                  ...
                  IIcon {
                    ...
                    Layout.rightMargin: Style.padding.small
    ...
    RowLayout {
      id: rowLayout
      anchors.fill: parent
      anchors.topMargin: Style.padding.small
      spacing: 0
        ...
        ColumnLayout {
          ...
          Repeater {
            ...
            delegate: Rectangle {
              ...
              IDivider {
                anchors.centerIn: parent
                width: parent.width - Style.padding.small
