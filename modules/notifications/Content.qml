...
Item {
  ...
  implicitWidth: Style.notifications.width + Style.padding.small * 2
  implicitHeight: {
    const count = list.count;
    if (count === 0)
      return 0;

    let height = (count - 1) * Style.spacing.small;
    for (let i = 0; i < count; i++)
      height += list.itemAtIndex(i)?.nonAnimHeight ?? 0;

    return Math.min((QsWindow.window?.screen?.height ?? 0) - Settings.appearance.thickness * 2, height + Style.spacing.small * 2);
  }
  ...
  ClippingWrapperRectangle {
    ...
    IListView {
      ...
      delegate: Item {
        ...
        implicitHeight: notif.implicitHeight + (idx === 0 ? 0 : Style.spacing.small)
        ...
        ClippingRectangle {
          anchors.top: parent.top
          anchors.topMargin: wrapper.idx === 0 ? 0 : Style.spacing.small
      ...
      IExtraIndicator {
        ...
        extra: {
          ...
          let height = 0;
          for (let i = 0; i < count; i++) {
            height += (list.itemAtIndex(i)?.nonAnimHeight ?? 0) + Style.spacing.small;

            if (height - Style.spacing.small >= scrollY)
              return i;
          }
