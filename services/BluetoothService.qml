  ...
  Connections {
    ...
    function onStateChanged() {
      ...
      if (bluetoothBlockedToggled) {
        ...
        ToastService.showNotice("Bluetooth", "Bluetooth enabled", "bluetooth");
        discoveryTimer.running = true;
      } else {
        ToastService.showNotice("Bluetooth", "Bluetooth disabled", "bluetooth-off");
      }
    }
  }
    ...
    stdout: StdioCollector {
      onStreamFinished: {
        ...
        if (wifiBlocked && wifiBlocked === root.blocked) {
          ...
          ToastService.showNotice("Airplane mode", "Airplane mode enabled", "plane");
        } else if (!wifiBlocked && wifiBlocked === root.blocked) {
          root.airplaneModeToggled = true;
          NetworkService.setWifiEnabled(true);
          ToastService.showNotice("Airplane mode", "Airplane mode disabled", "plane-off");
        } else if (adapter.enabled) {
          ToastService.showNotice("Bluetooth", "Bluetooth enabled", "bluetooth");
          discoveryTimer.running = true;
        } else {
